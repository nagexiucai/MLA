var nbv = (function() {
  ...
  function render_ipynb(obj, target, settings) {
    ...
    t.setAttribute('style', ['max-width: 960px', 'border: 1px solid #ccc', 'margin: 1em auto', 'padding: 1.5em 1.5em 1.5em 7em', 'background-color: white', 'box-shadow: 0 0 10px #ccc'].join(';'));
  ...
  function excount(cell, tin) {
    ...
    cc.setAttribute('style', ['display: block', 'position: absolute', 'top: ' + (tin ? '5px' : '0 !important'), 'left: -7.5em', 'width: 7em', 'font-family: monospace', 'text-align: right', 'color: ' + (tin ? '#303fba' : '#de4815')].join(';'));
    cc.textContent = (tin ? 'In' : 'Out') + ' [' + ((!cell.execution_count && !cell.prompt_number) ? ' ' : (cell.execution_count || cell.prompt_number)) + ']:';
    return cc;
  }
  ...
  function handle_code(cell) {
    ...
    // for excount positioning

    el.appendChild(excount(cell, true));

    var pre = d.createElement('pre');
    // because code may also be within markdown:
    pre.setAttribute('style', ['background: #f7f7f7', 'border: 1px solid #cfcfcf', 'padding: .4em', 'margin-bottom: 0', 'margin-top: 0', 'border-radius: 2px', 'min-height: .85em'].join(';'));
    ...
    for (var j = 0; j < cell.outputs.length; j++) {
      ...
      case 'pyout':
        ...
        break;
      default:
        console.error('Not supported output_type: ' + cell.outputs[j].output_type);
      }
    ...
    for (var j = 0; j < fmts.length; j++) {
      ...
      case 'text/html':
        ...
        for (var k = 0; k < dfs.length; k++) {
          dfs[k].setAttribute('style', ['border-collapse: collapse', 'text-align: left'// 'margin-top: 1em'
          ].join(';'));
      ...
      default:
        if (fmt.startsWith('image/')) {
          ...
          dm.setAttribute('src', 'data:' + fmt + ';base64,' + ((typeof dt.data[fmt]) == 'string' ? dt.data[fmt] : dt.data[fmt].join('')));
          break;
        }
  ...
  function handle_stream_output(dt) {
    ...
    var outt = dt.name || dt.stream;
    // v4 || v3; contains 'stdout' or 'stderr'
